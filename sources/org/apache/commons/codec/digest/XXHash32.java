...
public class XXHash32 implements Checksum {
    ...
    public long getValue() {
        int i = 0;
        int rotateLeft = (this.stateUpdated ? Integer.rotateLeft(this.state[0], 1) + Integer.rotateLeft(this.state[1], 7) + Integer.rotateLeft(this.state[2], 12) + Integer.rotateLeft(this.state[3], 18) : this.state[2] + PRIME5) + this.totalLen;
        int i2 = this.pos - 4;
        while (i <= i2) {
        ...
        while (i < this.pos) {
            rotateLeft = Integer.rotateLeft(rotateLeft + ((this.buffer[i] & 255) * PRIME5), 11) * PRIME1;
            i++;
        }
        int i3 = ((rotateLeft >>> 15) ^ rotateLeft) * PRIME2;
