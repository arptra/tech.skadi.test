...
public abstract class FragmentPagerAdapter extends PagerAdapter {
    ...
    public static String a(int i, long j) {
        return "android:switcher:" + i + AccountConstantKt.CODE_SEPARTOR + j;
    }
    ...
    public Object instantiateItem(@NonNull ViewGroup viewGroup, int i) {
        ...
        Fragment p0 = this.mFragmentManager.p0(a(viewGroup.getId(), itemId));
        if (p0 != null) {
            this.mCurTransaction.i(p0);
        } else {
            p0 = getItem(i);
            this.mCurTransaction.c(viewGroup.getId(), p0, a(viewGroup.getId(), itemId));
        }
        if (p0 != this.mCurrentPrimaryItem) {
            p0.setMenuVisibility(false);
            if (this.mBehavior == 1) {
                this.mCurTransaction.x(p0, Lifecycle.State.STARTED);
            } else {
