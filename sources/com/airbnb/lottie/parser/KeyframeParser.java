...
class KeyframeParser {
    static JsonReader.Options INTERPOLATOR_NAMES = JsonReader.Options.of(x.f, "y");
    private static final Interpolator LINEAR_INTERPOLATOR = new LinearInterpolator();
    private static final float MAX_CP_VALUE = 100.0f;
    static JsonReader.Options NAMES = JsonReader.Options.of(t.f, s.f, "e", "o", ProtocolVersions.PROTOCOL_KEY_IDENTIFIER, "h", "to", "ti");
    ...
    private static WeakReference<Interpolator> getInterpolator(int i) {
        ...
        synchronized (KeyframeParser.class) {
            weakReference = (WeakReference) pathInterpolatorCache().g(i);
        }
    ...
    private static Interpolator interpolatorFor(PointF pointF, PointF pointF2) {
        ...
        if (interpolator2 == null || interpolator == null) {
            try {
                ...
                a2 = "The Path cannot loop back on itself.".equals(e.getMessage()) ? PathInterpolatorCompat.a(Math.min(pointF.x, 1.0f), pointF.y, Math.max(pointF2.x, 0.0f), pointF2.y) : new LinearInterpolator();
            }
            interpolator = a2;
    ...
    private static void putInterpolator(int i, WeakReference<Interpolator> weakReference) {
        synchronized (KeyframeParser.class) {
            pathInterpolatorCache.m(i, weakReference);
        }
    }
}
