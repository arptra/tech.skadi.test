...
public class TextLayer extends BaseLayer {
    ...
    private void drawGlyphTextLine(String str, DocumentData documentData, Font font, Canvas canvas, float f, float f2, float f3) {
        for (int i = 0; i < str.length(); i++) {
            FontCharacter fontCharacter = (FontCharacter) this.composition.getCharacters().g(FontCharacter.hashFor(str.charAt(i), font.getFamily(), font.getStyle()));
    ...
    private void offsetCanvas(Canvas canvas, DocumentData documentData, int i, float f) {
        ...
        float f2 = 0.0f;
        float f3 = (((float) i) * documentData.lineHeight * dpScale) + (pointF == null ? 0.0f : (documentData.lineHeight * dpScale) + pointF.y);
    ...
    private List<TextSubLine> splitGlyphTextIntoLines(String str, float f, Font font, float f2, float f3, boolean z) {
        ...
        for (int i4 = 0; i4 < str.length(); i4++) {
            ...
            if (z) {
                FontCharacter fontCharacter = (FontCharacter) this.composition.getCharacters().g(FontCharacter.hashFor(charAt, font.getFamily(), font.getStyle()));
                if (fontCharacter == null) {
                } else {
