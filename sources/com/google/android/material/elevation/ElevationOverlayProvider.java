...
public class ElevationOverlayProvider {
    ...
    private boolean isThemeSurfaceColor(@ColorInt int i) {
        return ColorUtils.k(i, 255) == this.colorSurface;
    }
    ...
    public int compositeOverlay(@ColorInt int i, float f) {
        ...
        int layer = MaterialColors.layer(ColorUtils.k(i, 255), this.elevationOverlayColor, calculateOverlayAlphaFraction);
        if (calculateOverlayAlphaFraction > 0.0f && (i2 = this.elevationOverlayAccentColor) != 0) {
            layer = MaterialColors.layer(layer, ColorUtils.k(i2, OVERLAY_ACCENT_COLOR_ALPHA));
        }
        return ColorUtils.k(layer, alpha);
    }
