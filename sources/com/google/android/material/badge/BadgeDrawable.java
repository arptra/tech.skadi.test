...
public class BadgeDrawable extends Drawable implements TextDrawableHelper.TextDrawableDelegate {
    ...
    private void calculateCenterAndBounds(@NonNull Rect rect, @NonNull View view) {
        ...
        if (badgeGravity2 == 8388659 || badgeGravity2 == 8388691) {
            this.badgeCenterX = ViewCompat.z(view) == 0 ? (((float) rect.left) - this.halfBadgeWidth) + ((float) totalHorizontalOffsetForState) : (((float) rect.right) + this.halfBadgeWidth) - ((float) totalHorizontalOffsetForState);
        } else {
            this.badgeCenterX = ViewCompat.z(view) == 0 ? (((float) rect.right) + this.halfBadgeWidth) - ((float) totalHorizontalOffsetForState) : (((float) rect.left) - this.halfBadgeWidth) + ((float) totalHorizontalOffsetForState);
        }
