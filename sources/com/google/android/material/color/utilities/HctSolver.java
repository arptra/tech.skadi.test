...
public class HctSolver {
    ...
    public static int findResultByJ(double d, double d2, double d3) {
        ...
        while (true) {
            ...
            double d6 = cos;
            double pow2 = Math.pow(((d2 == 0.0d || sqrt == 0.0d) ? 0.0d : d2 / Math.sqrt(d5)) * pow, 1.1111111111111112d);
            double aw = (viewingConditions.getAw() * Math.pow(d5, (d4 / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double d7 = (((0.305d + aw) * 23.0d) * pow2) / (((23.0d * d6) + ((pow2 * 11.0d) * cos2)) + ((pow2 * 108.0d) * sin));
            double d8 = d7 * cos2;
            double d9 = d7 * sin;
