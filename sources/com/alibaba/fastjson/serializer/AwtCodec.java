...
public class AwtCodec implements ObjectSerializer, ObjectDeserializer {
        ...
        while (jSONLexer.token() != 13) {
            if (jSONLexer.token() == 4) {
                ...
                if (JSON.DEFAULT_TYPE_KEY.equals(stringVal)) {
                    ...
                    if (stringVal.equalsIgnoreCase(x.f)) {
                        i2 = i;
        ...
        while (jSONLexer.token() != 13) {
            if (jSONLexer.token() == 4) {
                ...
                if (stringVal.equalsIgnoreCase(x.f)) {
                    ...
                    i3 = i;
                } else if (stringVal.equalsIgnoreCase(MonthView.VIEW_PARAMS_WIDTH)) {
                    i4 = i;
                } else if (stringVal.equalsIgnoreCase(MonthView.VIEW_PARAMS_HEIGHT)) {
                    i5 = i;
                } else {
                    throw new JSONException("syntax error, " + stringVal);
                }
    ...
    public void write(JSONSerializer jSONSerializer, Object obj, Object obj2, Type type, int i) throws IOException {
        ...
        if (obj instanceof Point) {
            ...
            serializeWriter.writeFieldValue(writeClassName(serializeWriter, Point.class, '{'), x.f, point.x);
            serializeWriter.writeFieldValue((char) StringUtil.COMMA, "y", point.y);
        } else if (obj instanceof Font) {
            Font font = (Font) obj;
            serializeWriter.writeFieldValue(writeClassName(serializeWriter, Font.class, '{'), "name", font.getName());
            serializeWriter.writeFieldValue((char) StringUtil.COMMA, "style", font.getStyle());
            serializeWriter.writeFieldValue((char) StringUtil.COMMA, "size", font.getSize());
        } else if (obj instanceof Rectangle) {
            Rectangle rectangle = (Rectangle) obj;
            serializeWriter.writeFieldValue(writeClassName(serializeWriter, Rectangle.class, '{'), x.f, rectangle.x);
            serializeWriter.writeFieldValue((char) StringUtil.COMMA, "y", rectangle.y);
            serializeWriter.writeFieldValue((char) StringUtil.COMMA, MonthView.VIEW_PARAMS_WIDTH, rectangle.width);
            serializeWriter.writeFieldValue((char) StringUtil.COMMA, MonthView.VIEW_PARAMS_HEIGHT, rectangle.height);
        } else if (obj instanceof Color) {
            Color color = (Color) obj;
            serializeWriter.writeFieldValue(writeClassName(serializeWriter, Color.class, '{'), "r", color.getRed());
            serializeWriter.writeFieldValue((char) StringUtil.COMMA, "g", color.getGreen());
            serializeWriter.writeFieldValue((char) StringUtil.COMMA, "b", color.getBlue());
            if (color.getAlpha() > 0) {
                serializeWriter.writeFieldValue((char) StringUtil.COMMA, "alpha", color.getAlpha());
            }
        } else {
