...
class PinyinFormatter {
    private static String convertToneNumber2ToneMark(String str) {
        ...
        if (!lowerCase.matches("[a-z]*[1-5]")) {
            return lowerCase.replaceAll(ProtocolVersions.PROTOCOL_KEY_CONNECTION_VERSION, "ü");
        }
        ...
        stringBuffer.append(lowerCase.substring(0, indexOf).replaceAll(ProtocolVersions.PROTOCOL_KEY_CONNECTION_VERSION, "ü"));
        stringBuffer.append(charAt);
        stringBuffer.append(lowerCase.substring(indexOf + 1, lowerCase.length() - 1).replaceAll(ProtocolVersions.PROTOCOL_KEY_CONNECTION_VERSION, "ü"));
    ...
    public static String formatHanyuPinyin(String str, HanyuPinyinOutputFormat hanyuPinyinOutputFormat) throws BadHanyuPinyinOutputFormatCombination {
        ...
        if (HanyuPinyinToneType.WITHOUT_TONE == hanyuPinyinOutputFormat.getToneType()) {
            ...
            str = convertToneNumber2ToneMark(str.replaceAll("u:", ProtocolVersions.PROTOCOL_KEY_CONNECTION_VERSION));
        }
        if (HanyuPinyinVCharType.WITH_V == hanyuPinyinOutputFormat.getVCharType()) {
            str = str.replaceAll("u:", ProtocolVersions.PROTOCOL_KEY_CONNECTION_VERSION);
