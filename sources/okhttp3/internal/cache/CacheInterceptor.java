...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(@NotNull Interceptor.Chain chain) throws IOException {
        ...
        if (networkRequest == null && cacheResponse == null) {
            Response build = new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(OpenRemoteStarryNetAppCode.CODE_SEND_MESSAGE_FAIL).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1).receivedResponseAtMillis(System.currentTimeMillis()).build();
