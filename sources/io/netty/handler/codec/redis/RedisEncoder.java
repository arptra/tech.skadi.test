...
public class RedisEncoder extends MessageToMessageEncoder<RedisMessage> {
    ...
    private void writeArrayMessage(ByteBufAllocator byteBufAllocator, ArrayRedisMessage arrayRedisMessage, List<Object> list) {
        if (arrayRedisMessage.isNull()) {
            writeArrayHeader(byteBufAllocator, arrayRedisMessage.isNull(), -1, list);
            return;
        }
        writeArrayHeader(byteBufAllocator, arrayRedisMessage.isNull(), (long) arrayRedisMessage.children().size(), list);
        for (RedisMessage writeRedisMessage : arrayRedisMessage.children()) {
            writeRedisMessage(byteBufAllocator, writeRedisMessage, list);
        }
    }
    ...
    private void writeBulkStringHeader(ByteBufAllocator byteBufAllocator, BulkStringHeaderRedisMessage bulkStringHeaderRedisMessage, List<Object> list) {
        ...
        if (bulkStringHeaderRedisMessage.isNull()) {
            ...
            ioBuffer.writeBytes(numberToBytes((long) bulkStringHeaderRedisMessage.bulkStringLength()));
    ...
    private void writeFullBulkStringMessage(ByteBufAllocator byteBufAllocator, FullBulkStringRedisMessage fullBulkStringRedisMessage, List<Object> list) {
        ...
        ioBuffer2.writeBytes(numberToBytes((long) fullBulkStringRedisMessage.content().readableBytes()));
    ...
    private static void writeString(ByteBufAllocator byteBufAllocator, RedisMessageType redisMessageType, String str, List<Object> list) {
        ByteBuf ioBuffer = byteBufAllocator.ioBuffer(redisMessageType.length() + ByteBufUtil.utf8MaxBytes((CharSequence) str) + 2);
        redisMessageType.writeTo(ioBuffer);
        ByteBufUtil.writeUtf8(ioBuffer, (CharSequence) str);
        ioBuffer.writeShort(RedisConstants.EOL_SHORT);
        list.add(ioBuffer);
    }
    ...
    public void encode(ChannelHandlerContext channelHandlerContext, RedisMessage redisMessage, List<Object> list) throws Exception {
        try {
