...
public class ChineseToPinyin {
    ...
    private int getChsAscii(String str) {
        ...
        try {
            ...
            if (bytes == null || bytes.length > 2 || bytes.length <= 0) {
                Log.e("lijinqian", "ChineseToPinyin $ getChsAscii illegal resource string : " + str);
                throw new RuntimeException("illegal resource string");
            }
            byte b2 = bytes.length == 1 ? bytes[0] : 0;
            try {
                if (bytes.length != 2) {
                    return b2;
                }
                ...
                e = e;
                b = b2;
                Log.e("lijinqian", "ChineseToPinyin $ getChsAscii exception : " + str);
                e.printStackTrace();
                return b;
            }
        } catch (Exception e2) {
    ...
    public String convertAll(String str) {
        ...
        while (i < str.length()) {
            int i2 = i + 1;
            int chsAscii = getChsAscii(str.substring(i, i2));
            if (chsAscii <= 0 || chsAscii >= 160) {
                int length = pyvalue.length - 1;
                while (true) {
                    if (length < 0) {
                        break;
                    } else if (pyvalue[length] <= chsAscii) {
                        str2 = str2 + pystr[length];
                        break;
                    } else {
                        length--;
                    }
                ...
                str2 = str2 + String.valueOf((char) chsAscii);
            }
            i = i2;
        }
        return str2;
    ...
    public String getSelling(String str) {
        ...
        while (i < str.length()) {
            ...
            if (substring.getBytes().length == 2 && (substring = convert(substring)) == null) {
                substring = StarryNetConstant.DEVICE_NAME_UNKNOWN;
            }
    ...
    public String toLower(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            str2 = str2 + toLower(str.charAt(i));
        }
        return str2;
    }

    public String toUper(String str) {
        String str2 = "";
        for (int i = 0; i < str.length(); i++) {
            str2 = str2 + toUper(str.charAt(i));
        }
        return str2;
    }
}
