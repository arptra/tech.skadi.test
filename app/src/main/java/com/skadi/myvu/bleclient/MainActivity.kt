...
class MainActivity : AppCompatActivity(), BleManager.Listener {
    ...
    override fun onStateChanged(state: BleState) {
        runOnUiThread {
            ...
            disconnectButton.isEnabled = state is BleState.Connecting || state is BleState.ServicesDiscovering || state is BleState.EnablingNotifications || state is BleState.MtuNegotiation || state is BleState.HandshakeSent || state is BleState.WaitFirstVendorNotify || readyState
        }
    }
